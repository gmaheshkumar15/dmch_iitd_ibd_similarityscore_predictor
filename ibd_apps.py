...
try:
    ...
    svc_model = joblib.load('svc_model_only.pkl')
    xgb_model = joblib.load('xgb_model_only.pkl')

    scaler1 = joblib.load("scaler_final1.pkl")
    scaler2 = joblib.load("scaler_final2.pkl")
    scaler3 = joblib.load("scaler_final3.pkl")
...
# -----------------------------
# EXTRA CSS TO MAKE MODEL NAMES + VALUES BIGGER
# -----------------------------
st.markdown("""
<style>
.big-label {
...
This tool is developed by DMCH Ludhiana and IIT Delhi. It uses machine learning (ML) models to estimate the similarity of a diet with those consumed by patients prior to an Inflammatory Bowel Disease (IBD) diagnosis.It uses Logistic Regression, Support Vector Machine and XGBoost Classifiers for this purpose. 
...
if uploaded_file:
    try:
        ...
        df_scaled1 = scaler1.transform(df_merged)
        df_scaled2 = scaler2.transform(df_merged)
        df_scaled3 = scaler3.transform(df_merged)

        # Step 3: Predictions
        log_prob = log_model.predict_proba(df_scaled1)[:, 1]
        svc_prob = svc_model.predict_proba(df_scaled2)[:, 1]
        xgb_prob = xgb_model.predict_proba(df_scaled3)[:, 1]

        df_predictions = df_merged.copy()
        df_predictions["Logistic_Prob"] = log_prob
        df_predictions["SVC_Prob"] = svc_prob
        df_predictions["XGB_Prob"] = xgb_prob
        ...
        with col_right:
            ...
            # -------------------------------
            # ONLY CHANGE → BIGGER TEXT
            # -------------------------------
            st.markdown(
        f"<div class='big-label'>Logistic Regression:      "
        f"<span class='big-score'>{log_prob[0] * 100:.0f}</span></div>",
        unsafe_allow_html=True)

            st.markdown(
        f"<div class='big-label'>Support Vector Classifier: "
        f"<span class='big-score'>{svc_prob[0] * 100:.0f}</span></div>",
        unsafe_allow_html=True)

            st.markdown(
        f"<div class='big-label'>XGBoost Classifier:      "
        f"<span class='big-score'>{xgb_prob[0] * 100:.0f}</span></div>",
        unsafe_allow_html=True
    )
        ...
        ws.append(["XGBoost Classifier", round(xgb_prob[0] * 100, 0)])
